...
import { Backdrop, Box, CircularProgress, ImageListItem } from "@mui/material";
...
const LobbyPage = () => {
	...
	return (
		<Box className={style.container}>
			...
			<Box className={style.bg_img}></Box>
			...
			{isModalPlayersOpened && <Box className={style.modal__players}>
				...
				<Box className={style.nums}>
					{buttons.map((btn, key) => <NumberItem key={key} id={btn.number} onClick={handleClick} number={btn.number}
						active={btn.active} />)}
				</Box>
				...
				<Box className={style.modal__playes_bg}></Box>
			</Box>}
			{isModalRoomOpened &&
				<Box className={style.modal__join}>
					<Box className={style.content}>
						<Box className={style.modal_img}></Box>
						<label htmlFor="room-number" style={{ color: '#FF0000' }}>enter a room number</label>
						<Box className={style.modal_icon}></Box>
						<input className={style.modal_input} onChange={handleChangeRoomNumber} id='room-id-input' pattern="[0-9]*" inputMode="numeric" name="room-number" maxLength={4}
							placeholder="enter a room number" />
						<Box className={style.modal_buttons}>
							<Button id='join-r' onClick={joinRoom} className={style.drink}>join lobby</Button>
							<Button id='cancel-r' onClick={() => handleCancel(1)} className={style.cancel}>cancel</Button>
						</Box>
					</Box>
				</Box>}
		</Box>
